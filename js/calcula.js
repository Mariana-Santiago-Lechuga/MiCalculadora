...
function sin() { //funcion seno
         x=Math.sin(x);
         pantalla.innerHTML=x; //mostrar en pantalla
         igualar() //resolver y mostrar operaciones pendientes
         xi=1 //reiniciar la pantalla
         }
function cos() { //funcion coseno
         x=Math.cos(x);
         pantalla.innerHTML=x; //mostrar en pantalla
         igualar() //resolver y mostrar operaciones pendientes
         xi=1 //reiniciar la pantalla
         }
function tan() { //funcion tangente
         x=Math.tan(x);
         pantalla.innerHTML=x; //mostrar en pantalla
         igualar() //resolver y mostrar operaciones pendientes
         xi=1 //reiniciar la pantalla
        }
function logNat() { //funcion logaritmo natural
         x=Math.log (x);
         pantalla.innerHTML=x; //mostrar en pantalla
         igualar() //resolver y mostrar operaciones pendientes
         xi=1 //reiniciar la pantalla
...
//funciones de conversion de decimal
function binario(){  // conversion decimal a binario
         DecBin = x.toString(2); //base 2
         pantalla.innerHTML=DecBin;
         op="no"; //quitar operaciones pendientes.
         xi=1; // reiniciar la pantalla.
        DecBin=0;
    
         }
 function octal(){  // conversion decimal a octal
         DecOc = x.toString(8); // base 8
         pantalla.innerHTML=DecOc;
         op="no"; //quitar operaciones pendientes.
         xi=1; //reiniciar la pantalla.
        DecOc=0;
          }
 function hexadecimal(){ // conversion decimal a hexadecimal
          DecHex = x.toString(16); // base 16
          pantalla.innerHTML=DecHex;
          op="no"; //quitar operaciones pendientes.
          xi=1; //reiniciar la pantalla.
         DecHex=0;
          }
